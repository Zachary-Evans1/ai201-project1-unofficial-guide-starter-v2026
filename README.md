...
Mechanism: The results contain source filenames, and generate.py tells Gemini to name the source. The generated answers do not consistently follow that instruction. The same question can produce an answer with a source in one run adn not have a source in another, even though retrieval is deterministic. The fact that results change through different runs points towards generation rather than retrieval.

### Miss for Criterion 5:
Stage: Chunking

Mechanism: The paragraph-based chunker creates many small chunks because it treats ever paragraph and heading as a separate chunk. Some documents have heading that become their own chunks, and many paragraphs are shorter than the 500-character maximum I set. As a result the 88 documents became 271 chunks instead of staying under the 200-chunk target.
